-- 🛠️ Script de Correção de Schema - Versão Simplificada
...
-- 2. Adicionar coluna order_date na tabela orders (se não existir)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3. Adicionar coluna status na tabela customers (se não existir)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- 4. Criar tabela financial_records (se não existir)
DO $$
BEGIN
  IF NOT EXISTS (
